...
class Jax2TfTest(tf_test_util.JaxToTfTestCase):
  ...
  def test_convert_of_jaxlayer_fprop(self):
    ...
    # Calculate
    output_opt = np.array(test_input) @ np.array(params['w'])
    ...
    self.assertAllClose(output, output_opt, rtol=1e-1, atol=0)
    self.assertNotEmpty(output)

  def test_convert_of_jaxlayer_fprop_outside_context(self):
    input_dims = 10
    output_dims = 5
    batch_size = 3
    ...
    try:
      ...
      logging.warning("Output can not be generated outside of context.")
